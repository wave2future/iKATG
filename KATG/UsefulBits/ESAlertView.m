...
- (void)_handleRotation:(NSNotification *)note;
	...
	if ((self = [super init]))
	{
		...
		if (message)
		{
			...
			else
			{
				...
				if ([UIFont instancesRespondToSelector:@selector(lineHeight)])
					lineHeight		=	[[UIFont systemFontOfSize:kMessageFontSize] lineHeight];
				else
					lineHeight		=	[@"ASDF" sizeWithFont:[UIFont systemFontOfSize:kMessageFontSize]].height;
		...
		[self _handleRotation:nil];
		[[NSNotificationCenter defaultCenter] addObserver:self 
												 selector:@selector(_handleRotation:) 
...
- (void)_handleRotation:(NSNotification *)note
{
	...
	switch (orientation) {
		case UIDeviceOrientationPortraitUpsideDown:
			rotation	=	CGAffineTransformMakeRotation(ToRadians(180.0));
			break;
		case UIDeviceOrientationLandscapeLeft:
			rotation	=	CGAffineTransformMakeRotation(ToRadians(90.0));
			break;
